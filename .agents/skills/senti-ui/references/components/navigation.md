...
| `768 – 1023px` | 导航仍在底部，各 item **自动挂 `parallel`**：切横排（icon 左、文字右）、active 整行药丸背景，bar 内部上下各留 `0.571em` 内边距 |
...
- `click`：用户点击时冒泡到宿主（`bubbles` + `composed`），active 切换由外部处理（写法同 st-tab-bar，见 tabs 文档）。
