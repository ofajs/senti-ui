...
- **技能内示例代码必须 ofa.js API 优先**：ofa 页面场景一律用模板绑定语法（`{{xxx}}` / `attr:xxx`（布尔属性必须 `attr:`） / `sync:value` / `sync:open` / `on:click`）或 ofa 数据读写（`el.value` 等已反射的运行时状态）；原生 JS 写法（`setAttribute` / `removeAttribute` / `addEventListener`）只能作为"非 ofa 环境 / 自动化测试"的兼容示例出现，不得作为推荐写法
