...
- `click` / `input` / `change` / `close` 等在 senti-ui 组件上均已 composed，直接在宿主标签上监听即可（st-input/st-textarea 内部已把非 composed 的原生 change 转发出来）
- 监听方式：ofa 页面模板用 `on:click="method"`；纯 JS 场景用 ofa 的选择器与事件 API `$("st-button").on("click", fn)`——**不要写 `document.querySelector(...).addEventListener(...)`**
