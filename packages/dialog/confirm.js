...
// 用法：
//   const ok = await stConfirm("确定删除吗？");           // → true / false / null（遮罩、Escape）
//   const ok = await stConfirm({ title, message, yes, cancel, color }); // color 如 "error" 用于危险操作
import { escapeHtml, createDialog } from "./util.js";

export default async function stConfirm(options) {
  let message = "";
  let title = "";
  let yesText = "确定";
  let cancelText = "取消";
