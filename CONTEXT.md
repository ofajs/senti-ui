...
35. **靠尺寸拼合的视觉必须给底层同色兜底**——用"边框扩展成实心底"（如 st-switch 选中态）、多层覆盖等技巧时，em 换算在非基准字号下产生小数像素，设备像素取整会露出 1px 缝（st-switch 曾在 12px/20px 字号的轨道正中露出浅灰细线）；正确写法：被覆盖的底层（track 背景）同步设为同一颜色，取整残行露出来也不可见
